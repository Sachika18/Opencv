...
2.  **Download the OpenCV for Android SDK:** Obtain the SDK from the [official OpenCV website](https://opencv.org/releases/).
...
1.  **Capture:** The `CameraPreview` class captures frames from the device's camera using the Camera2 API's `ImageReader`.
2.  **Process:** Each frame (in YUV_420_888 format) is passed to the `processFrame` native function. In C++, the Y-plane of the image is converted into an OpenCV `Mat` object. The Canny edge detection algorithm is then applied.
3.  **Render:**
